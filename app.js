...
$('label').each(function () {
    let tabLetter = $(this).html().split('');
    $(this).html('');

    for (let x = 0; x < tabLetter.length; x++) {
        $(this).append("<span>" + tabLetter[x] + "</span>");
    }

    $(this).mouseover(function () {
        let letter = $(this).children('span');
        let interval = (x) => {
            setTimeout(() => {
    ...
    if (flag) {
        let interval = (x) => {
            setTimeout(() => {
                ...
                if (x === 90) {
                    $(this).attr('src', 'https://media.nouvelobs.com/ext/uri/sreferentiel.nouvelobs.com/file/rue89/5f41cf3fb6d4f6598fae9c1903fb5bbb.jpg');
                }
            }, 10);
        }
        interval(0);
...
$.get( "./ul.JSON", function(data) {

    for (let element of data.ul) {
